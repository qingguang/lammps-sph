...
function preproc() {
    ...
    ${restart2data} poly.restart poly.txt
    awk -v Nbeads=${Nbeads} -v Nsolvent=${Nsolvent} -v Npoly=full \
	-f addpolymer_bond.awk poly.txt > polyd.txt
nangles=$(tail -n 1 polyd.txt | awk '{print $1}')
nbounds=$(awk '/Angles/{exit} NF' polyd.txt | tail -n 1 | awk '{print $1}')
sed -e "s/_NUMBER_OF_ANGLES_/$nangles/1" -e "s/_NUMBER_OF_BOUNDS_/$nbounds/1" polyd.txt > poly.txt
}
